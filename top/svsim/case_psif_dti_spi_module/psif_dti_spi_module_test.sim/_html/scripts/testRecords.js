...
function processTestdetailsData(data) {
	...
	rowItems.attribute = usrAttr[0];
        if (usrAttr[1].toString().indexOf("</a>") > -1) { // If External Anchor Link
            rowItems.value = "";
            rowItems.link = usrAttr[1]
        } else {
        rowItems.value = usrAttr[1];
        }
	detailsRowData.push(rowItems);
	});
    }
        ...
        },
        {
            ...
            cellRenderer: function (params) {
                if (params.data.attribute === 'Date') {
                    ...
                    var result = parseAnchorLink(params.data.link);
                    var divExtrAttr = 'href="' + result[0] + '"';
