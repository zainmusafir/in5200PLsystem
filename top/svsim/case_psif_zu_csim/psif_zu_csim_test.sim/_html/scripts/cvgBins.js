...
function topPageDescription(instance) {
    $('#page-header-text').text(GLOBAL_JSON.prod +  reportType + '  Coverage Summary');
...
function processBinsData(g_data) {
    cvpLength = 0;
    reportType = (urlParams.hasOwnProperty('ty') && urlParams.ty == 4) ? ' Power Aware Bins' : ' Covergroups';
...
function processCoverpointsMapData(g_data) {
    ...
    if (cvps === undefined || cvpLength === cvps.length ) {
        ...
        $('table#desc-table').append(
            '<tr>' +
        ((isOpenedInCvgPage) ? '<th>' + ((gbinsData.head[1] == COVERGROUPS_IDs.CROSS) ? 'Selected Cross' : 'Selected Coverpoint') + '</th>' : '<th>' + ((gbinsData.head[1] == COVERGROUPS_IDs.CROSS) ? 'Cross' : 'Coverpoint') + '</th>') +
            '<td>' + gbinsData.head[0]  + '</td>' +
        '</tr>'
        );
        ...
        if (normalBinData.length > 0) {
            $('.nav-tabs').append(getTemplate('tab', 'bin',  (urlParams.hasOwnProperty('ty') && urlParams.ty == 4 ) ? 'Bins' : 'Auto, Default and User Defined Bins'));
