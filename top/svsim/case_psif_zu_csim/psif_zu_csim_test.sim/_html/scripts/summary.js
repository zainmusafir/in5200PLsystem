...
function topPageDescription(instance, type) {
    if (type == 'du') {
        $('#page-header-text').text(GLOBAL_JSON.prod + ' Design Unit Coverage');
        $('#page-header-container h4').html('<span style="color:#b0b3bc;"> Design Unit: </span>' + instance);
    } else {
        $('#page-header-text').text(GLOBAL_JSON.prod + (isPA ? ' Power Aware Instance' : ' Instance' ) + ' Coverage');
        $('#page-header-container h4').html('<span style="color:#b0b3bc;"> Instance: </span>' + instance);
    }
...
function covSummaryByInstance(id, dataObj, type) {
    ...
    ['a', 'b', 'fc', 'g', 'd', 'fe', 'fs', 'ft', 's', 't'].forEach(function (covType) {
    ...
    if (dataObj.hasOwnProperty('loc')) {
        ...
        for (var covType in dataObj.loc.data) {
            if (dataObj.loc.data.hasOwnProperty(covType)) {
                rowData[0].children[0][covType] = (covType == 'g') ? (dataObj.loc.data[covType][1]) : (typeof dataObj.loc.data[covType][0] === 'string') ? (dataObj.loc.data[covType][0]) : adjustPrecision((dataObj.loc.data[covType][1] / dataObj.loc.data[covType][0] * 100));
            }
        }
    } else {
    ...
    for (covType in dataObj.rec.data) {
        if (dataObj.rec.data.hasOwnProperty(covType)) {
            rowData[0][covType] = (covType == 'g') ? (dataObj.rec.data[covType][1]) : (typeof dataObj.rec.data[covType][0] === 'string') ? (dataObj.rec.data[covType][0]) : adjustPrecision((dataObj.rec.data[covType][1] / dataObj.rec.data[covType][0] * 100));
        }
    }
...
function getRowData(data, type) {
    ...
    Object.keys(data).sort().forEach(function (covType) {
        if (data.hasOwnProperty(covType)) {
            var functionalCoverage =  (covType === 'gb' || covType === 'g' || covType === 'cvpc' || covType === 'a' || covType === 'd');
            if (!( isPAInstance || isPA) ) {  // In case of Coverage report or non-PA Instance report Show the Columns grouped
                if (!(covType === 'gb' || covType === 'cvpc' || covType === 'f')) {
                    var rowData = {
                        ...
                        cp: (covType === 'g') ? data[covType][DATA_ARR.HITS_INDEX] : adjustPrecision(data[covType][DATA_ARR.HITS_INDEX] / data[covType][DATA_ARR.BINS_INDEX] * 100)
                    };

                    if (data[covType].length > 2) {
                    ...
                    // if (!functionalCoverage) {
                    //     rowData.s = urlParams.s;
                    // } else {
                    //     if (type && type == 'du') {
                    //         rowData.s =  urlParams.hasOwnProperty('oneInst') ? urlParams.oneInst : urlParams.s;
                    //     } else {
                    //         rowData.s = urlParams.s;
                    //     }
                    // }
            ...
            } else { // In case of PA report Show the Columns flat and Covergroup bins will renamed to power bins
                if (!(covType === 'g' || covType === 'cvpc')) {

                    rowData = {
                        name: (covType === 'gb') ? 'Power Aware bins' : (covType === 'a') ? 'Power Aware Checks' :  coverageMapper[covType],
                        type: covType,
                        bins: data[covType][DATA_ARR.BINS_INDEX],
                        hit: data[covType][DATA_ARR.HITS_INDEX],
                        miss: data[covType][DATA_ARR.BINS_INDEX] - data[covType][DATA_ARR.HITS_INDEX],
                        cp: adjustPrecision(data[covType][DATA_ARR.HITS_INDEX] / data[covType][DATA_ARR.BINS_INDEX] * 100)
                    };
                    if (type) {
                        if (data[covType].length > 2) {
                            rowData.f = covType === 'gb' ? data['g'][DATA_ARR.FILE_NUM_INDEX] : data[covType][DATA_ARR.FILE_NUM_INDEX];
                            if (typeof data[covType][DATA_ARR.FILE_SUB_NUM_INDEX] !== 'undefined') {
                                rowData.fsub = data[covType][DATA_ARR.FILE_SUB_NUM_INDEX];
                            }
                        } else if (data[covType].length > 1 && typeof data[covType][0] === 'string' && (typeof data[covType][1] !== 'string') ) {
        ...
        columnDefs.push(
            {
                ...
                cellRendererParams: {
                    innerRenderer: function (params) {
                        if (params.data.f) {
                            return '<a href="' + (params.data.type == 'gb' ? pageName['g'] : pageName[params.data.type]) + 'f=' + params.data.f + (params.data.hasOwnProperty('fsub') ? ('&fsub=' + params.data.fsub) : '') + (params.data.hasOwnProperty('pr') ? ('&pr=' + params.data.pr) : '') + (urlParams.hasOwnProperty('type') ? '&type=' + urlParams.type : '') + '&s=' + params.data.s + '&cp=' + params.data.cp + '">' + params.value + '</a>';
                        } else {
                            return params.value;
                        }
                    },
        ...
        columnDefs.push(
            {
                ...
                cellRendererParams: {
                    innerRenderer: function (params) {
                        if (params.data.f && (params.data.type != 'gb' && params.data.type != 'cvpc')) {
