    ...
    if (consolidatedView) {
        ...
        if (urlParams.hasOwnProperty('fsub') && urlParams.f == urlParams.fsub) {
            if (g_data.hasOwnProperty(urlParams.s + '_sub')) {
                $.merge(dataObj[urlParams.s][urlParams.t == PREFIX.ASSERTIONS ? 'asserts' : 'direct'], g_data[urlParams.s + '_sub'][urlParams.t == PREFIX.ASSERTIONS ? 'asserts' : 'direct']);
            }
        }
...
function topPageDescription(instance) {
    $('#page-header-text').text(GLOBAL_JSON.prod  + ' ' + covType + ' Coverage Report');
            ...
            Object.keys(data).forEach(function(id) {
                ...
                if (data.hasOwnProperty(id) && !( dataObj[id].ty == 2 && GLOBAL_JSON.command === 'coverage' && urlParams.type === 'du')) {
                    ...
                    if (!consolidatedView || (consolidatedView && (data[id].ty == consolidatedViewType || data[id].ty == TYPE.BOTH))) {
                        data[id].asserts.forEach(function(assert) {
                            var currentRowData = {
                                name: (consolidatedView) ? (((data[id].ty == TYPE.BOTH) ? ((consolidatedViewType == TYPE.INSTANCE) ? data[id].pr : data[id].dun) : (data[id].pr)) + ( consolidatedViewType == TYPE.INSTANCE ? '/' : '::') + assert.n) : assert.n,
                ...
                if (data.hasOwnProperty(id) && !( dataObj[id].ty == 2 && GLOBAL_JSON.command === 'coverage' && urlParams.type === 'du')) {
                    if (!consolidatedView || (consolidatedView && (data[id].ty == consolidatedViewType || data[id].ty == TYPE.BOTH))) {
                        data[id].direct.forEach(function(direct) {
                            rowData.push({
                                name: (consolidatedView) ? (((data[id].ty == TYPE.BOTH) ? ((consolidatedViewType == TYPE.INSTANCE) ? data[id].pr : data[id].dun) : (data[id].pr)) + ( consolidatedViewType == TYPE.INSTANCE ? '/' : '::') + direct.n) : direct.n,
                                hits: direct.h,
