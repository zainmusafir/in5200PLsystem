...
function processTpLinks(g_data) {
    ...
    createDataGridInstance('testplan', testplanColumnDefs, getRowData(g_data), {
...
function getRowData(data) {
    ...
    Object.keys(data.tp).forEach(function (item) {
        if ((data.tp[item].fixed_attr_val).length === 11) {
            if (data.tp[item].fixed_attr_val[TESTPLAN_ITEM.LEVEL] === 1) {
                parentId = 0;
                ...
                parentId = (data.tp[item].fixed_attr_val[TESTPLAN_ITEM.SECTION_NUMBER]).slice(0, index);
                Idn = data.tp[item].fixed_attr_val[TESTPLAN_ITEM.SECTION_NUMBER];
            }
        }
        var rowItems = {
...
function initializeData(usr_attr) {
    testplanColumnDefs = [
        {
