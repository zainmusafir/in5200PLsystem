...
function processCovergroupsData(g_data) {
    ...
    if (isDataLoaded) {
        ...
        if (dataObj[urlParams.s] && dataObj[urlParams.s].hasOwnProperty('ty') && dataObj[urlParams.s].ty == 4) {
...
function topPageDescription(instance) {
    $('#page-header-text').text(GLOBAL_JSON.prod +  reportType + reportScope + ' Coverage Report');
...
function getData(dataObj) {
    ...
    for (var key in dataObj) {
        ...
        if (dataObj.hasOwnProperty(key) && !( dataObj[key].ty == 2 && GLOBAL_JSON.command === 'coverage' && urlParams.type === 'du')) {
            ...
            if (!consolidatedView || (consolidatedView && (element.ty == consolidatedViewType || element.ty == TYPE.BOTH))) {
                if (element.hasOwnProperty('cvgs')) {
                    var cvgTypes = element.cvgs.filter(function (e) {
                        return (e.h[1] === COVERGROUPS_IDs.COVERGROUP_TYPE || // Covergroup
                                ( (isPA || isPACombined ) && e.h[1] === COVERGROUPS_IDs.COVERGROUP_INSTANCE  )
                        );
...
function constructDataObj(cvg, extraData) {
    ...
    var obj = {
        ...
        opts: cvg.opts,
        c: cvg.cov[3]  //Exclusion Comment if exist
    };
        ...
        details.addEventListener('click', function (e) {
            ...
            if (params.data.type === COVERGROUPS_IDs.COVERPOINT || params.data.type === COVERGROUPS_IDs.CROSS) {
                switch (e.which) {
                    case 1:
                        ...
                        if (typeof href !== 'undefined') {
                            ...
                            href = hrefBins + '&dt=' + (params.data.type === COVERGROUPS_IDs.COVERPOINT ? 'cp' : 'cc');
                            $('#frameCont').remove();
                ...
                if (e.which == 1) {
                    ...
                    for (var gridOptions in gridOptionsDetails) {
                        if (gridOptionsDetails.hasOwnProperty(gridOptions)) {
                            if (typeof gridOptionsDetails[gridOptions] !== 'undefined') {
                                clearUrlParamsByPrefix(urlParams, gridOptions == 'crosses' ? PREFIX.CROSS : PREFIX.COVERPOINT);
                    ...
                    headerTemplate += '</span> &nbsp; <span class="table-cell-td" style="word-break: break-all;">' + (e.currentTarget.innerText || e.currentTarget.innerHTML) + '</span> <br/> <br/>';
                    headerTemplate += '</div>';
...
function processCoverpointsData (g_data) {
    ...
    if (coverPointsData.length != 0) {
        ...
        $('.nav-tabs').append(getTemplate('tab', 'coverPoints', 'coverpoints'));
...
function initializeData(type, cvgOpts) {
    ...
    cvgColumnDefs[0].headerName = 'Covergroups';
    cvgColumnDefs[0].headerTooltip = 'Covergroups';
    ...
    cvgPointsColumnDefs[0].headerName = 'Coverpoints';
    cvgPointsColumnDefs[0].headerTooltip = 'Coverpoints';
