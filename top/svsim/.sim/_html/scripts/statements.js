...
function topPageDescription(instance) {
    $('#page-header-text').text(GLOBAL_JSON.prod  + ' Statements Coverage Report');
        ...
        file.cov.forEach(function(stmt) {
            stmt[STATEMENTS_SRC.ITEMS_INFO].forEach(function(item) {
                var currentRowData = {
                    ...
                    item: item[STATEMENTS_SRC.ITEM_INDEX],
...
function processSrcEditorData() {
    ...
    if (dataObj.stmts.length === 1) {
        tabId = 0;
        for (var h = 1; h <= dataObj.stmts[tabId][STATEMENTS_SRC.MAX_ITEMS]; h++) {
...
function getItemInfo(itemsInfo) {
    ...
    var uniqueItems = Object.keys(uniqueItemsFlags).sort();
