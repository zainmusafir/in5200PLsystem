...
function topPageDescription() {
    $('#page-header-text').text(GLOBAL_JSON.prod + (isPA ? ' Power Aware' : '') + ' Coverage Report Summary');
...
function checkGrids() {
    ...
    if (structExist) { // If File Size < 50 Instance structure table exists
        ...
        if (trExist) { // If test chart exists
            ...
            duDivId = 'du-summary';
            instDivId = duExist ? 'instance-summary' : 'tests-summary'; // check if the instance summary will be moved to the top row or stay in the bottom row alongside the du-summary
        }
        ...
        duDivId = instExist ? 'du-summary' : 'structure-summary';
        instDivId =  duExist ? 'instance-summary' : 'structure-summary';
    }
...
function getSummaryTypeData(data, type) {
    ...
    Object.keys(data).sort().forEach(function (covType) {
        if (data.hasOwnProperty(covType)) {
            if (!(covType === 'f' || covType === 'tc' || covType === 'gb' || covType === 'cvpc')) {
                var rowData = {
                ...
                if (covType === 'a' || covType === 'd' || covType === 'g' )  {
                    rowData.type = type;
                }
