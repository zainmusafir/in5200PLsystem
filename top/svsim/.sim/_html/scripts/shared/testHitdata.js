...
function hitsRenderer(params, thd) {
    ...
    if (params.data[thd.fileNum] && params.data[thd.scope] &&
        params.value != 0     // testhitdata should not be shown if hits == 0
    ) {
        return thdRenderer(params, thd);
    //Exclusion Renderer
    } else if (exclusion.hasComment || exclusion.excludedStatus || exclusion.hasReason) {
...
function exclRenderer(params, exclusion) {
    ...
    var cellTemplate = $(
        ...
        ( exclusionIcon ? //Adding icon in case of exclusion comment or exclusion reason
            ('<i class="fa fa-info-circle" aria-hidden="true" data-toggle="popover" title="Exclusion comment : '+(params.data.c ?  params.data.c : params.data.exc ) +'" style="color:#2361a0" id="item' + params.rowIndex + '"></i></div>') : ('</div>'))
    );
